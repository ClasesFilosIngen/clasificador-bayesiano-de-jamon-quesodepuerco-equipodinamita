...
def bagOfWords(listaOraciones): #hacemos la bolsa de palabras 
	#v=separar(listaOraciones) #mandamos a separar cada conjunto de palabras
	bag={}
...
#esperando que no se modifique el newSpam
	aux=newSpam.copy()
	probDic(aux, contSpam)
	print(newSpam)
	newSpam=aux
#esperando que no se modifique el newSpam
	aux=bagUniverse.copy()
	probDic(aux, contUniverse)
	newUniverse=aux
#p(spam| "secreto")
	pSecretoSpam=primerClasificacion(token, newSpam, probSpam, newUniverse)
	if isinstance(pSecretoSpam,str):
		print(pSecretoSpam)
	else:
		print("p(|spam"+token+")=%0.5f" % pSecretoSpam)
#p(ham|secreto)
	pSecretoHam=primerClasificacion(token, newHam, probHam, newUniverse)
	if isinstance(pSecretoHam,str):
		print(pSecretoHam)
	else:
		print("p(|spam"+token+")=%0.5f" % pSecretoHam)
